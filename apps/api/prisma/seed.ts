...
function safeGet(obj: any, pathArr: string[], fallback: any = null) {
  try {
...
const getNumber = (field: any) => {
  ...
  if (typeof field === "string") return field.trim() === "" ? null : Number(field);
  if (typeof field?.value === "number") return field.value;
  if (typeof field?.value === "string") return field.value.trim() === "" ? null : Number(field.value);
  return null;
};
...
async function upsertVendor(vendorObj: any) {
  const name =
    safeGet(vendorObj, ["value", "vendorName", "value"]) ||
...
async function upsertCustomer(customerObj: any) {
  const name =
    safeGet(customerObj, ["value", "customerName", "value"]) ||
...
async function createInvoiceFrom(raw: any) {
  ...
  const invoiceNumber =
    safeGet(invBlock, ["value", "invoiceId", "value"]) ||
    safeGet(raw, ["analyticsId"]) ||
    null;

  /* ------------------ FIXED: Smart Invoice Date Extraction ------------------ */
  let invoiceDate =
    toDate(safeGet(invBlock, ["value", "invoiceDate", "value"])) ||
  ...
  /* ------------------ Line Items ------------------ */
  const items =
    safeGet(llm, ["lineItems", "value", "items", "value"]) ||
