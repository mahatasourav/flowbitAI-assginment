...
router.get("/stats", async (req, res) => {
    try {
        ...
        const spendThisMonth = thisMonthInvoices.reduce((sum, inv) => sum + (inv.invoiceTotal ?? 0), 0);
        const spendLastMonth = lastMonthInvoices.reduce((sum, inv) => sum + (inv.invoiceTotal ?? 0), 0);
        const invoiceCountThisMonth = thisMonthInvoices.length;
        const invoiceCountLastMonth = lastMonthInvoices.length;
        const avgThisMonth = invoiceCountThisMonth ? spendThisMonth / invoiceCountThisMonth : 0;
        const avgLastMonth = invoiceCountLastMonth ? spendLastMonth / invoiceCountLastMonth : 0;
        return res.json({
            totalSpendYTD,
            totalInvoices,
            totalDocuments,
